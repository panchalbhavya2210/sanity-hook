...
const client = createClient({
  projectId: projectId,
  dataset: dataset,
  useCdn: false, // We want the latest data, not cached CDN data
  apiVersion: apiVersion,
});

async function main() {
  try {
    console.log(`Fetching all data from Sanity Project: ${projectId}, Dataset: ${dataset}`);
    ...
    console.log(`Successfully fetched ${data.length} documents and saved to data/data.json`);
  } catch (err) {
